...
public static class SchedulingEndpoints
{
        ...
        IPropertyDirectory properties, string? date, string? serviceId, CancellationToken ct)
    {
        ...
        var live = (await repo.ListOverlappingAsync(
                ctx.TenantId, ctx.PropertyId, dayStartUtc.AddHours(-12), dayStartUtc.AddDays(1).AddHours(12), ct))
        ...
        for (var minutes = profile.OpenMinute; minutes + duration <= profile.CloseMinute; minutes += 30)
        {
        ...
        HttpContext http, SchedulingService scheduling, IAppointmentRepository repo, IClock clock, CancellationToken ct)
    {
        ...
        var current = await repo.GetAsync(ctx.TenantId, ctx.PropertyId, req.AppointmentId!, ct);
        ...
        var result = await scheduling.PreflightAsync(ctx.TenantId, ctx.PropertyId, current, proposal, ct);
        ...
        string id, string body, CancellationToken ct)
    {
        ...
        var outcome = await scheduling.CommitMoveAsync(
            ctx.TenantId, ctx.PropertyId, id, req.Token!, req.Reason, ctx.Actor, ctx.CorrelationId, ct);
